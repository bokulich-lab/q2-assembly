...
def _map_sample_reads(
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        raise ValueError(os.listdir(bam_result))
    ...
    # This is a formality because QIIME 2 methods are required to return at
    # least one artifact, so we need a valid instance of that Artifact to return
    return result
...
def _gather_sample_data(
    ...
    """
    reads_manifest = reads.manifest.view(pd.DataFrame)
    full_set = {}
...
def map_reads_to_contigs(
    ...
    full_sample_set = _gather_sample_data(indexed_contigs, reads, paired)

    result = BAMDirFmt()
    result = Artifact.import_data("SampleData[AlignmentMap]", result, validate=None)
    for samp, props in full_sample_set.items():
        _map_sample_reads(result, common_args, paired, samp, props)

    raise ValueError(os.listdir(result.path))
    return (result,)
