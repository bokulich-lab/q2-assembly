...
from q2_assembly.iss.iss import (_process_iss_arg, _rename_reads_files,
                                 _generate_reads, _abundances_to_biom,
                                 generate_reads)
...
class TestISS(TestPluginBase):
    ...
    def generate_exp_calls(self, samples, dest):
        ...
        for s in samples:
            ...
            call_ext.extend([
                '--output', os.path.join(dest, f'{s}_00_L001')
            ])
    ...
    def test_abundances_to_biom(self):
        abunds_fp = sorted(glob.glob(
            os.path.join(self.get_data_path('abundances'), '*.txt')))
    ...
    def test_generate_reads_x(self, p1, p2, p3):
        ...
        pd.testing.assert_frame_equal(
            _table_to_dataframe(obs_abundances),
            _table_to_dataframe(exp_biom_table)
        )
