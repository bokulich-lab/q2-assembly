...
class TestMegahit(TestPluginBase):
    package = 'q2_assembly.megahit.tests'

    def setUp(self):
        super().setUp()
        self.fake_common_args = ['--presets', 'meta-fake', '--k-min', '39']
        self.test_params_dict = {
            'presets': 'meta-sensitive', 'min_count': 3, 'k_list': [15, 35],
            'k_min': 17, 'k_max': 73, 'k_step': 14,
            'no_mercy': True, 'bubble_level': 1, 'prune_level': 1,
            'prune_depth': 3, 'disconnect_ratio': 0.6, 'low_local_ratio': 0.1,
            'max_tip_len': 3, 'cleaning_rounds': 4, 'no_local': False,
            'kmin_1pass': True, 'memory': 0.75, 'mem_flag': 1,
            'num_cpu_threads': 2, 'no_hw_accel': False, 'min_contig_len': 100
        }
        self.test_params_list = [
            '--presets', 'meta-sensitive', '--min-count', '3', '--k-list',
            '15,35', '--k-min', '17', '--k-max', '73', '--k-step', '14',
            '--no-mercy', '--bubble-level', '1', '--prune-level', '1',
            '--prune-depth', '3', '--disconnect-ratio', '0.6',
            '--low-local-ratio', '0.1', '--max-tip-len', '3',
            '--cleaning-rounds', '4', '--kmin-1pass', '--memory', '0.75',
            '--mem-flag', '1', '--num-cpu-threads', '2', '--min-contig-len',
            '100'
        ]
    ...
    def test_process_sample_single_end(self, p1, p2):
        ...
        shutil.copy(
            contigs,
            os.path.join(test_temp_dir.name, 'results', 'final.contigs.fa')
        )
        ...
        exp_cmd = ['megahit', '-r', 'fwd_reads.fastq.gz',
                   '-o', os.path.join(test_temp_dir.name, 'results'),
                   '--presets', 'meta-fake', '--k-min', '39']
    ...
    def test_process_sample_paired_end(self, p1, p2):
        ...
        exp_cmd = ['megahit', '-1', 'fwd_reads.fastq.gz',
